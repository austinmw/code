...
The following diagram from wikipedia (see wiki) shows the complete merge sort process for 
...
access the memory at (x + i*4). Unlike arrays, we can not do random access in linked list. Quick 
Sort requires a lot of this kind of access. In linked list to access i’th index, we have to travel 
...
4. Used in External Sorting (external data memory location)
*/