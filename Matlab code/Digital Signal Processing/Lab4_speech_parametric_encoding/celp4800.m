function [xhat,e,k,theta0,P,b] = celp4800(x,N,L,M,c,cb,Pidx)
...
for (f=1:F)
  ...
  sigma  = udecode(uencode(sigma,4),4);
  kappa  = sin(pi/2*sigma);
  theta0 = udecode(uencode(theta0,2,0.2),2,0.2);
  b      = udecode(uencode(b,3,1.4),3,1.4);
